...
def encode_bytes(data: str) -> bytes:
    ...
    checksum = f"{encoder.hash.digest:0{CHECKSUM_SIZE}x}".encode("ascii")

    out = bytearray(HEADER_SIZE + len(encoded_payload))
    out[:6] = HEADER_MAGIC
    out[6:HEADER_SIZE] = checksum
    out[HEADER_SIZE:] = encoded_payload
    return bytes(out)
...
def decode_bytes(raw_bytes: bytes) -> str:
    ...
    if len(raw_bytes) < HEADER_SIZE or raw_bytes[:6] != HEADER_MAGIC:
        raise DGDataDecodeError("Invalid file header: Not a valid DGDATA save format")

    decoder = Decoder(-1, raw_bytes[6:HEADER_SIZE])
    decoded = decoder.digest(raw_bytes[HEADER_SIZE:])

    calculated = f"{decoder.hash.digest:0{CHECKSUM_SIZE}x}".encode("ascii")
    if calculated != decoder.checksum:
        raise DGDataDecodeError(
            f"Checksum verification failed: expected {decoder.checksum.decode('ascii', errors='replace')}, "
            f"calculated {calculated.decode('ascii', errors='replace')} (file may be corrupted or truncated)"
        )
