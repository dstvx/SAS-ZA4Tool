"""DGDATA Encoding and Decoding Library for SAS4 (NumPy Vectorized Implementation).

This module provides functionality to encode and decode data using the DGDATA format,
which involves a custom checksum and byte obfuscation algorithm using NumPy vectorization.
"""
...
# Precomputed table for the custom CRC-32-like checksum
BYTEHASH_TABLE: Final[list[int]] = [
    0x00000000,
    0x09073096,
    0x120E612C,
    0x1B0951BA,
...
class DGDataHash:
    ...
    def update(self, data: np.ndarray) -> None:
        ...
        if not data.size:
            return
        digest = self.digest
        table = BYTEHASH_TABLE
        for b in data.tobytes():
            digest = (digest >> 8) ^ table[(digest ^ b) & 0xFF]
...
class Encoder:
    ...
    def digest(self, data: bytes) -> bytes:
        ...
        data_array = frombuffer(data, dtype=uint8)
        self.hash.update(data_array)

        # Vectorized encoding
        idx = (np.arange(len(data_array)) + self.data_cursor) % 6
        encoded = ((data_array + idx + 21) & 0xFF).astype(uint8)

        self.data_cursor += len(data_array)
...
class Decoder:
    ...
    def digest(self, data: bytes) -> bytes:
        ...
        # Vectorized decoding
        idx = (np.arange(len(data_array)) + self.data_cursor) % 6
        decoded = ((data_array - idx - 21) & 0xFF).astype(uint8)
        decoded_bytes = decoded.tobytes()

        self.hash.update(frombuffer(decoded_bytes, dtype=uint8))
        self.data_cursor += len(data_array)
        return decoded_bytes


def encode_bytes(data: str) -> bytes:
    """Encodes cleartext string into signed DGDATA bytes in memory using NumPy."""
    ...
    return HEADER_MAGIC + checksum + encoded


def decode_bytes(raw_bytes: bytes) -> str:
    """Decodes in-memory DGDATA bytes and verifies checksum using NumPy."""
...
def encode_to_file(data: str, file_path: str) -> None:
    """Encodes a string to a file in DGDATA format using NumPy."""
    data_bytes = data.encode()
    encoder = Encoder(len(data_bytes))
    header = zeros(HEADER_SIZE, dtype=uint8)
    header[:6] = frombuffer(HEADER_MAGIC, dtype=uint8)
    ...
    with open(file_path, "wb") as f:
        ...
        checksum = f"{encoder.hash.digest:0{CHECKSUM_SIZE}x}".encode()
        f.seek(-(len(encoded) + len(checksum)), os.SEEK_CUR)
        f.write(checksum)


def decode_from_file(filename: str) -> str:
    """Decodes a DGDATA formatted file into a string using NumPy."""
    with open(filename, "rb") as f:
        header = fromfile(f, dtype=uint8, count=HEADER_SIZE)
        if header[:6].tobytes() != HEADER_MAGIC:
            raise DGDataDecodeError("Invalid file header")
        data = fromfile(f, dtype=uint8)

    decoder = Decoder(-1, header[6:].tobytes())
    decoded = decoder.digest(data.tobytes())

    calculated = f"{decoder.hash.digest:0{CHECKSUM_SIZE}x}".encode()
    if calculated != bytes(decoder.checksum):
        raise DGDataDecodeError("Checksum verification failed")
